...
public enum LogCategory: String {
   case view, controller, animation, io, net, db, `extension`, window
}
...
open class Logger {
   ...
   public static var format = Format.middle
   ...
   public static func getLogger(_ category: LogCategory) -> Logger {
      return loggerType.init(category.rawValue)
   }

   private var lock = os_unfair_lock() // Not used in Production builds.
   ...
   open func initialize(_ message: String, function: StaticString = #function, file: StaticString = #file, line: UInt = #line, dso: UnsafeRawPointer? = #dsohandle) {
   ...
   open func deinitialize(_ message: String, function: StaticString = #function, file: StaticString = #file, line: UInt = #line, dso: UnsafeRawPointer? = #dsohandle) {
      guard Self.level >= .verbose else {
         return
      }
      log(level: .verbose, category: name, message: "(Memory) ~~~ \(message)", function: function, file: file, line: line, dso: dso)
