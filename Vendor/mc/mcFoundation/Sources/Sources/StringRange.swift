...
public struct StringRange {
   ...
   public init(string: String, uncheckedRange range: UTF16Range, shouldEraseIndexesOnExactReplace: Bool = true) {
      self.shouldEraseIndexesOnExactReplace = shouldEraseIndexesOnExactReplace
      self.string = string
      startIndexOffset = range.lowerBound
      endIndexOffset = range.upperBound
   }
}
...
extension StringRange {
   ...
   public var uncheckedRange: UTF16Range {
      return NSRange(lowerBound: startIndexOffset, upperBound: endIndexOffset)
   }
