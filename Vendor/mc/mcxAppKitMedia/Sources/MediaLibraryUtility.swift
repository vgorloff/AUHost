...
//  MCA-OSS-AUH
//
//  Created by Vlad Gorlov on 6/24/15.
...
private let log = Logger.getLogger(MediaLibraryUtility.self)
...
public final class MediaLibraryUtility: NSObject {
   ...
   override public init() {
      ...
      observation = mediaLibrary.observe(\.mediaSources) { [weak self] object, _ in guard let this = self else { return }
         ...
         log.verbose("Found \(sources.count) media sources: \(Array(sources.keys))")
