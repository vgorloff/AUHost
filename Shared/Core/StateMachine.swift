...
public struct StateMachineGraph<State, Event, Context> {
...
public struct StateMachine<State, Event, Context> {
   ...
   private let graph: StateMachineGraph<State, Event, Context>

   public typealias OldState = State
   public typealias NewState = State

   public private(set) var state: State
   public var stateChangeHandler: ((OldState, Event, NewState) -> Void)?

   public init(context: Context, graph: StateMachineGraph<State, Event, Context>) {
      self.context = context
      self.graph = graph
      self.state = graph.initialState
   }

   public mutating func handleEvent(event: Event) throws {
